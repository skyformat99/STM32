...
/****************************************************************************
* 名    称：void RS485_Send_Data(USART_TypeDef* USARTx,uint16_t Data)
* 功    能： 485发送
* 入口参数：无
* 出口参数：无
* 说    明：
* 调用方法：无 
****************************************************************************/
void RS485_Send_Data(USART_TypeDef* USARTx,uint16_t Data)
{ 
...
void USART_OUT(USART_TypeDef* USARTx, uint8_t *Data,...){ 
	...
	while(*Data!=0){				                          //判断是否到达字符串结束符
		if(*Data==0x5c){									  //'\'
			switch (*++Data){
				case 'r':							          //回车符
					RS485_Send_Data(USARTx, 0x0d);	   

					Data++;
					break;
				case 'n':							          //换行符
					RS485_Send_Data(USARTx, 0x0a);	
					Data++;
					break;
		...
		else RS485_Send_Data(USARTx, *Data++);
		while(USART_GetFlagStatus(USARTx, USART_FLAG_TC)==RESET);
	}
...
void GPIO_Configuration(void)
{
  ...
  GPIO_ResetBits(GPIOD, GPIO_Pin_12);		                     //RS485输出模式 禁止485输入
